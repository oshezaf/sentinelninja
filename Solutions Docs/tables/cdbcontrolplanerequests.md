...
Reference for CDBControlPlaneRequests table in Azure Monitor Logs.
...
| BackupIntervalInMinutes | int | The time (in minutes) between consecutive backup snapshots for the Cosmos DB account. Valid range: 60 - 1440 minutes. |
| BackupRetentionIntervalInHours | int | The duration of time (in hours) for which backup snapshots are retained for the Cosmos DB account. Valid range: 8 - 720 hours. |
| BackupStorageRedundancy | string | The type of backup storage redundancy for the Cosmos DB account. |
...
| TargetBackupMode | string | The backup mode after update. This can be either Continuous or Periodic. |
| TargetContinuousTier | string | The continuous tier after updating. This can be either Continuous7Days or Continuous30Days. |
...
- [CDBControlPlaneRequests Schema Reference (Azure Monitor)](https://learn.microsoft.com/en-us/azure/azure-monitor/reference/tables/cdbcontrolplanerequests)
