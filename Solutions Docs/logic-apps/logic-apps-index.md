...
Logic Apps connectors and built-in actions referenced by Microsoft Sentinel playbooks. **Managed** connectors come from `Microsoft.Web/connections` resources backed by Microsoft-published APIs (`/providers/Microsoft.Web/locations/.../managedApis/...`). **Custom** connectors are solution-specific APIs (`/customApis/...`). **Built-in** rows are workflow actions of type `Http`, `Function`, `Workflow`, or `ApiManagement` that don't use a connection resource. Multiple action instances of the same connector within a playbook are aggregated.

| Metric | Count |
|:-------|------:|
| Managed connectors | 96 |
| Custom connectors | 56 |
| Built-in action types | 3 |
