...
| **Source Product** | Defender Threat Feed *(basis: title)* |
...
- **Virtual Network permissions (for private access)**: For private storage account access, **Network Contributor** permissions are required on the Virtual Network and subnets. The Function App subnet must be delegated to **Microsoft.App/environments** for Flex Consumption VNet integration.
