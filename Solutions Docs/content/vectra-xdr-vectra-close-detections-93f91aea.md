...
| Get_Access_Token_For_Remediated_Closing_Detections | get | `/secrets/@{encodeURIComponent('Vectra-Access-Token')}/value` | — |
| Get_Access_Token_For_Closing_Benign_Detections | get | `/secrets/@{encodeURIComponent('Vectra-Access-Token')}/value` | — |
...
| HTTP_Request_To_Close_Remediated_Detections | PATCH | `@{variables('base_url')}/api/@{variables('api_version')}/detections/close` | — |
| HTTP_Request_To_Close_Benign_Detections | PATCH | `@{variables('base_url')}/api/@{variables('api_version')}/detections/close` | — |
...
| GenerateAccessTokenVectra_For_Benign | — | — | workflowId=`[concat('/subscriptions/', subscription().subscriptionId, '/resourceGroups/',resourceGroup().name,'/providers/Microsoft.Logic/workflows/',trim(parameters('GenerateAccessCredPlaybookName')))]`<br>triggerName=`manual` |
