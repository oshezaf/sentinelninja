...
|:------|:-------------|:---------------:|:-------------:|:---------:|
| [`DeviceRegistryEvents`](../tables/deviceregistryevents.md) | `ActionType == "RegistryValueSet"`<br>`InitiatingProcessFileName == "explorer.exe"`<br>`RegistryKey endswith "RunMRU"`<br>`RegistryValueData contains "#"`<br>`RegistryValueData matchesregex "[^[:ascii]]"` | ✓ | ✗ | ? |
