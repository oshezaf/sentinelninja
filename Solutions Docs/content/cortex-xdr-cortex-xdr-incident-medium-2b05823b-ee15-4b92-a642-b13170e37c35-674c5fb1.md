# Cortex XDR Incident - Medium

A new incident was created in the Cortex XDR portal with a severity "Medium". Click on the events for incident details. 

| Attribute | Value |
|:----------|:------|
| **Type** | Analytic Rule |
| **Solution** | [Cortex XDR](../solutions/cortex-xdr.md) |
| **ID** | `2b05823b-ee15-4b92-a642-b13170e37c35` |
| **Severity** | Medium |
| **Kind** | Scheduled |
| **Required Connectors** | [CortexXDR](../connectors/cortexxdr.md) |
| **Source** | [View on GitHub](https://github.com/Azure/Azure-Sentinel/blob/master/Solutions/Cortex%20XDR/Analytic%20Rules/CortexXDR_Medium.yaml) |

## Tables Used

This content item queries data from the following tables:

- [`PaloAltoCortexXDR_Incidents_CL`](../tables/paloaltocortexxdr-incidents-cl.md)

---

**Browse:** [🏠](../readme.md) · [Solutions](../solutions-index.md) · [Connectors](../connectors-index.md) · [Tables](../tables-index.md) · [Content](../content/content-index.md) · [Parsers](../parsers/parsers-index.md) · [ASIM Parsers](../asim/asim-index.md) · [ASIM Products](../asim/asim-products-index.md)

↑ [Back to Analytic Rules](analytic-rules.md) · [Back to Cortex XDR](../solutions/cortex-xdr.md)

