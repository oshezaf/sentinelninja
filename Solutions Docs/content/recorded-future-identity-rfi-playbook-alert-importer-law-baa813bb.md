...
- *Create_table_if_missing*: method=`post`, path=`/api/logs`

**`azuremonitorlogs`** (managedApi):
- *Check_if_table_exists*: method=`post`, path=`/queryDataV2`
...
- *Playbook_Alerts_-_Update_Playbook_Alert_-_Saved_to_LAW*: method=`put`, path=`/playbook-alerts/update`
